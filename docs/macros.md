...
- [`cbs-katex.html`]\: a configuration defining [KaTeX] macros for use in web pages
- [`cbs-mathjax.html`]\: a configuration defining [MathJax] macros for use in web pages

The LaTeX package is easy to read, and includes some explanatory comments.
The KaTeX and MathJax definitions are encoded in JSON, to allow their direct use in configurations;
the need to double the backslashes reduces their readability.

Each file defines the same collection of LaTeX macro names, taking the same arguments.
The rendering of the CBS-LaTeX macros should have the same layout and symbols,
...
[`cbs-katex.html`]: katex/cbs-katex
[`cbs-mathjax.html`]: mathjax3/cbs-mathjax
